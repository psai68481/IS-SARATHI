...
import { Compass, FileDown, Layers, Menu, X, ShieldCheck } from 'lucide-react';
import { TenderPreset } from '@/types';

interface TopbarProps {
  activePreset: string;
  onSelectPreset: (presetId: string) => void;
  presets: TenderPreset[];
  onExportReport: () => void;
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
      <div className="px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
        ...
        {/* Right: Engine Status Badge & Export Report */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Live Engine Status Badge */}
          <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 shadow-sm">
